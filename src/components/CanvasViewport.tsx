  ...
  useEffect(() => {
    ...
    <linearGradient id="svg-rect-gloss" x1="0%" y1="0%" x2="100%" y2="100%">
      ...
      <stop offset="100%" stop-color="#000000" stop-opacity="0.18" />
        ...
        startHighResRecording: (heightPreset) => {
          isRecordingHighResRef.current = true;
          const originalWidth = containerRef.current?.clientWidth || 800;
          const originalHeight = containerRef.current?.clientHeight || 600;
          const scale = heightPreset / originalHeight;
          ...
          if (rendererRef.current) {
            rendererRef.current.setSize(originalWidth * scale, originalHeight * scale, false);
            rendererRef.current.setPixelRatio(1);
          }

          if (renderTargetRef.current) {
            renderTargetRef.current.setSize(originalWidth * scale, originalHeight * scale);
          }

          if (shaderMaterialRef.current) {
            shaderMaterialRef.current.uniforms.uResolution.value.set(originalWidth * scale, originalHeight * scale);
          }
        },
        stopHighResRecording: () => {
          ...
          if (renderTargetRef.current) {
            renderTargetRef.current.setSize(originalWidth * window.devicePixelRatio, originalHeight * window.devicePixelRatio);
          }

          if (shaderMaterialRef.current) {
            shaderMaterialRef.current.uniforms.uResolution.value.set(originalWidth, originalHeight);
          }
        }
      };
    }
